...
namespace basedx12 {
    ...
    void GameDivece::LoadAssets()
    {
        ...
        // Create the pipeline state, which includes loading shaders.
        {
            ...
            wstring v_shaderPath = App::GetRelativeShadersPath() + L"shader_mesh_simple_vert.cso";
            wstring p_shaderPath = App::GetRelativeShadersPath() + L"shader_mesh_simple_pixel.cso";
            wstring pa_shaderPath = App::GetRelativeShadersPath() + L"shader_mesh_alt_pixel.cso";
            // Load pre-compiled shaders.
            ThrowIfFailed(ReadDataFromFile(v_shaderPath.c_str(), &pVertexShaderData, &vertexShaderDataLength));
            ThrowIfFailed(ReadDataFromFile(p_shaderPath.c_str(), &pPixelShaderData1, &pixelShaderDataLength1));
            ThrowIfFailed(ReadDataFromFile(pa_shaderPath.c_str(), &pPixelShaderData2, &pixelShaderDataLength2));
        ...
        MeshUtill::CreateCube(1.0f, vertices, indeces);

        wstring dataPath = App::GetDataPath() + L"occcity.bin";
        ThrowIfFailed(ReadDataFromFile(dataPath.c_str(), &pMeshData, &meshDataLength));

        // Read in mesh data for vertex/index buffers.
        // Create the vertex buffer.
        {

            UINT vsize = (UINT)(sizeof(VertexPositionNormalTexture) * vertices.size());
        ...
        {
            UINT isize = (UINT)(sizeof(uint32_t) * indeces.size());
            ...
            m_numIndices = isize / 4;    // R32_UINT (SampleAssets::StandardIndexFormat) = 4 bytes each.
        }
        ...
        */
        ...
        // Create the depth stencil view.
        {
            ...
            ThrowIfFailed(m_device->CreateCommittedResource(
                &CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
                D3D12_HEAP_FLAG_NONE,
                &CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, App::GetGameWidth(), App::GetGameHeight(), 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL),
    ...
    void GameDivece::OnKeyDown(UINT8 key)
    {
        m_camera.OnKeyDown(key);
    }

    void GameDivece::OnKeyUp(UINT8 key)
    {
        m_camera.OnKeyUp(key);
    }

    // Create the resources that will be used every frame.
    void GameDivece::CreateFrameResources()
    {
    ...
    void GameDivece::PopulateCommandList(FrameResource* pFrameResource)
    {
