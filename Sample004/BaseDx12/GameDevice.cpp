...
namespace basedx12 {
    ...
    GameDivece* GameDivece::s_app = nullptr;
    ...
    void GameDivece::LoadAssets()
    {
...
#endif
            wstring shaderPath = App::GetRelativeShadersPath() + L"shaders.hlsl";
        ...
        UINT fileSize = 0;
        UINT8* pAssetData;
        wstring dataPath = App::GetDataPath() + L"SquidRoom.bin";

        ThrowIfFailed(ReadDataFromFile(dataPath.c_str(), &pAssetData, &fileSize));
    ...
    void GameDivece::LoadContexts()
    {
        ...
        struct threadwrapper
        {
            static unsigned int WINAPI thunk(LPVOID lpParameter)
            {
                ...
                GameDivece::Get()->WorkerThread(parameter->threadIndex);
                return 0;
            }
        };
    ...
    void GameDivece::OnUpdate()
    {
        ...
        if (m_keyboardInput.animate)
        {
            for (int i = 0; i < NumLights; i++)
            {
                ...
                m_lightCameras[i].Get3DViewProjMatrices(&m_lights[i].view, &m_lights[i].projection, 90.0f,
                    static_cast<float>(App::GetGameWidth()), static_cast<float>(App::GetGameHeight()));
            }
        }
    ...
    void GameDivece::WorkerThread(int threadIndex)
    {
    ...
    void GameDivece::SetCommonPipelineState(ID3D12GraphicsCommandList* pCommandList)
    {
