...
namespace basedx12 {
    ...
    void FixedTriangle::OnInitAssets() {
    ...
    void MoveTriangle::OnInitAssets() {
        auto Device = App::GetDx12Device();
        auto commandList = Device->GetCommandList();
        auto aspectRatio = Device->GetAspectRatio();
        D3D12_GRAPHICS_PIPELINE_STATE_DESC PipeLineDesc;
	...
	void Scene::OnInitAssets() {
        ...
        // それぞれのオブジェクトの初期化
        {
            m_FixedTriangle.OnInitAssets();
            m_MoveTriangle.OnInitAssets();
            m_MoveSquare.OnInitAssets();
        }
        //コンスタントバッファ
        {
            //コンスタントバッファハンドルを作成
