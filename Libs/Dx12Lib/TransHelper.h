...
namespace basedx12 {
	...
	//--------------------------------------------------------------------------------------
	struct AABB {
		bsm::Float3 m_Min;     ///< 小さいほうの座標
		bsm::Float3 m_Max;     ///< 大きいほうの座標
		//--------------------------------------------------------------------------------------
		/*!
		@brief	コンストラクタ
		*/
	...
	struct OBB{
		bsm::Float3 m_Center;     ///< 中心点の座標
		bsm::Float3 m_Rot[3];  ///< XYZ の各座標軸の傾きを表す方向ベクトル
		bsm::Float3 m_Size;     ///< OBB の各座標軸に沿った長さの半分（中心点から面までの長さ）
		//--------------------------------------------------------------------------------------
		/*!
	...
	struct PLANE{
		...
		*/
		...
		//--------------------------------------------------------------------------------------
		/*!
		@brief	コンストラクタ（bsm::Float4型から平面を作成）
		@param[in]	plane	bsm::Float4型の値
		*/
		//--------------------------------------------------------------------------------------
	...
	struct CYLINDER {
		float m_Radius;			///< 半径
		bsm::Float3 m_PointBottom;		///< 中間部線分の開始点
		bsm::Float3 m_PointTop;		///< 中間部線分の終了点
		//--------------------------------------------------------------------------------------
		/*!
		@brief	コンストラクタ
		*/
	...
	struct COLRECT {
		bsm::Float3 m_Center;     //中心点の座標
		bsm::Float3 m_Rot[2];  //XY の各座標軸の傾きを表す方向ベクトル
		...
		bsm::Float3 m_Vertex[4];	//行列によって変換された頂点(各種計算に使う)
		//--------------------------------------------------------------------------------------
		/*!
		@brief	コンストラクタ<br />
